...
<div class="flex-wrap flex relative">
  {#if loading}
    <div
      class="animate-spin border-4 border-t-blue-500 border-gray-300 rounded-full size-6"
    ></div>
  {:else}
  ...
  {#if error}
    <span
      class="text-red-500 border border-red-500 rounded-xl p-2 h-fit absolute top-8 bg-white"
